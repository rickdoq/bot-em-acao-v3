...
exports.run = (client, message, args) => {
  ...
  if (permissions.length == 0) {
    // caso esse membro n possua permissao alguma, vamos deixar a mensagem abaixo
  ...
  let embed = new Discord.MessageEmbed()
    ...
    .addField(
      ...
      `${member.roles.cache
        .filter(r => r.id !== message.guild.id)
        .map(roles => `<@&${roles.id}>`)
        .join(", ") || "Esse membro não possui cargos."}`,
      true
    );

  message.channel.send({ embed }).then(msg => {
    // um evento de reacao para fazermos adicionar outra embed (assim fica legal :3)
    msg.react("👉");
    ...
    coletor.on("collect", c => {
      ...
      msg.edit(emb).then(m => {
        // mais um evento, que caso o membro clique, retornara a primeira embed
        m.react("👈");
