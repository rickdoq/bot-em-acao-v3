    ...
    msg
        ...
        if (reaction.emoji.name === "1️⃣") {
            if("[\"1️⃣\"]" === JSON.stringify(item.resposta)){
                ...
                message.reply("parabéns! Você acertou e ganhou 50 coxinhas.");
                msg.reactions.removeAll(embed);
                //db.set(`work_${message.guild.id}_${user.id}`, Date.now());
                //db.add(`money_${message.guild.id}_${user.id}`, 50); // iremos adicionar 1 (um) na DB, que iremos usar como Programador
            }
            else{
                // Caso o usuário clique no emoji referente à Programador
